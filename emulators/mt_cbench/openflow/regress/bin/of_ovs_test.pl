...
push @ARGV, "--root=$ENV{'OFT_ROOT'}", "--common-st-args=ovs", "--controller=tcp:localhost:$of_port,tcp:localhost:$of_port", "--listener=tcp:127.0.0.1:6634", "--port_base=1", "--send_delay=2000", "--base_idle=2";

# Use a single random port instead of all four
push @ARGV, "--less_ports";

# Don't bother with VLAN if we go outside the box, too many issues...
#push @ARGV, "--no_vlan";
...
if ( defined($ENV{'OFT_OVS_MAP_ETH'}) ) {
    push @ARGV, "--map=$ENV{OFT_OVS_MAP_ETH}";
}
