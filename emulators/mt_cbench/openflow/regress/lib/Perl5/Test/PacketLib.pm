...
NF2::PacketLib.pm - A Perl module for manipulating packets for use in
...
This is the base NF2 module from which most of the more sophisticated packet 
modules inherit.
...
the nf_packet_in() and nf_expected_packet() functions defined in NF2::PacketGen.pm
...
sub new   # Ethernet_hdr
  {
    ...
    my $Ethernet_hdr = $class->NF2::PDU::new();
...
sub DA
  {
    my ($self, $val) = @_;
    if (defined $val) {
      @{$self->{'bytes'}}[0..5] = NF2::PDU::get_MAC_address($val);
    }
...
sub SA
  {
    my ($self, $val) = @_;
    if (defined $val) {
      @{$self->{'bytes'}}[6..11] = NF2::PDU::get_MAC_address($val);
    }
...
package NF2::VLAN_hdr;

use vars qw(@ISA);
@ISA = qw(NF2::Ethernet_hdr);
...
sub new   # VLAN_hdr
  {
    ...
    $VLAN_hdr->NF2::Ethernet_hdr::DA($arg{'DA'}) if (defined $arg{'DA'});
    $VLAN_hdr->NF2::Ethernet_hdr::SA($arg{'SA'}) if (defined $arg{'SA'});
...
Of these, priority and CFI are usually 0. NF2 uses the VID field to indicate the
...
e.g. This creates a VLAN header with an Ethertype indicating an IP packet (0x800)
  ...
  my $VLAN_hdr = NF2::VLAN_hdr->new( DA => $DA,
                                     SA => $SA,
                                     Ethertype => 0x800,
                                     VLAN_ID => 3
                                    );
...
sub SenderEthAddr
  {
    ...
    if (defined $val) {
      @{$self->{'bytes'}}[8..13] = NF2::PDU::get_MAC_address($val);
    }
...
sub TargetEthAddr
  {
    ...
    if (defined $val) {
      @{$self->{'bytes'}}[18..23] = NF2::PDU::get_MAC_address($val);
    }
...
use constant DEST_UNREACH_DATA_LEN => NF2::IP_pkt::IP_HDR_LEN + 8;
...
sub new_dest_unreach
  {
    ...
    $arg{'Type'} = NF2::ICMP->DEST_UNREACH;
    if (defined($arg{'Reason'})) {
      $arg{'Code'} = $arg{'Reason'};
    }
    else {
      $arg{'Code'} = NF2::ICMP::DEST_HOST_UNREACH;
    }
    ...
    if (defined($arg{'Packet'})) {
      ...
      if (!defined($arg{'SeqNo'})) {
        if ($arg{'Code'} == NF2::ICMP::DGRAM_TO_BIG) {
          $arg{'SeqNo'} = NF2::ICMP::NEXT_HOP_DEFAULT_MTU; # Next-Hop MTU
        }
        else {
          $arg{'SeqNo'} = 0;
        }
...
sub new_time_exceeded
  {
    ...
    $arg{'Type'} = NF2::ICMP->TIME_EXCEEDED;
    if (defined($arg{'Reason'})) {
      $arg{'Code'} = $arg{'Reason'};
    }
    else {
      $arg{'Code'} = NF2::ICMP::TTL_ZERO;
    }
    $arg{'Identifier'} = 0; # Unused
    $arg{'SeqNo'} = 0;
