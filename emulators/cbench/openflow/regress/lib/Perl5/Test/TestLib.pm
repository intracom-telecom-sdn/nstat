...
sub create_masks {
	...
	# ARP request/reply mask/data
	$arpMask = NF2::ARP_pkt->new(
		force     => 1,
		Ethertype => 0xffff,
		Op        => NF2::ARP->ARP_OP_MASK
	)->packed();

	$arpReqPkt = NF2::ARP_pkt->new( Op => NF2::ARP->ARP_REQUEST )->packed();

	$arpReplyPkt = NF2::ARP_pkt->new( Op => NF2::ARP->ARP_REPLY )->packed();

	#MDNS Mask
	$mdnsMask = NF2::IP_pkt->new(
		force      => 1,
	...
	$mdnsPkt = NF2::IP_pkt->new(
		dst_ip    => "224.0.0.251",
		Ethertype => 0x0800,
	)->packed();

	#IGMP Mask
	$igmpMask = NF2::IP_pkt->new(
		force      => 1,
		DA         => "00:00:00:00:00:00",
		dst_ip     => "255.255.255.255",
		Ethertype  => 0xffff,
	...
	$igmpPkt = NF2::IP_pkt->new(
		dst_ip    => "224.0.0.22",
		Ethertype => 0x0800,
	)->packed();

	# OSPF hello mask/data
	$ospfMask = NF2::IP_pkt->new(
		force      => 1,
	...
	$icmpPkt = NF2::IP_pkt->new( proto => 0x01, )->packed();

	# ICMP echo request
	$icmpEchoMask = NF2::ICMP_pkt->new(
		force         => 1,
		Ethertype     => 0xffff,
		version       => 0xf,
		ip_hdr_len    => 0x0,
	...
	$icmpEchoReqPkt = NF2::ICMP_pkt->new( Type => NF2::ICMP->ECHO_REQ, )->packed();

	$icmpEchoReplyPkt = NF2::ICMP_pkt->new( Type => NF2::ICMP->ECHO_REP, )->packed();
}
...
sub nftest_create_ICMP_echo_reply {
	...
	return NF2::ICMP_pkt->new_echo_reply( Request => $req );
}
...
sub nftest_pkt_cap_start {
  ...
  foreach my $iface (@$active_ports_ref) {
    ...
    # Create the capture thread
    push @threads, NF2::Pcap::start($ifaceReal, \&nftest_pkt_cap_arrival, $iface);
  }
...
sub nftest_start_vhosts {
	...
	foreach my $iface (@$active_ports_ref) {
		...
		# Create the capture thread
		push @threads, NF2::Pcap::start( $ifaceReal, \&nftest_vhost_pkt_arrival, $iface );
	}
...
sub nftest_send_IP {
	...
	if ($macKnown) {
		...
		if ( defined( $vhostsPendingPkts{$arpStr} ) ) {
			...
			my $arppkt = NF2::ARP_pkt->new_request(
				SA           => $srcMac,
				SenderIpAddr => $src,
...
sub nftest_send_ICMP_echo_req {
	...
	my $icmppkt = NF2::ICMP_pkt->new_echo_request(%arg);
...
sub processArpRequest {
	...
	my $arppkt = NF2::ARP_pkt->new_packed( Data => $pkt );
	...
	if ( defined( $vhostsIPiface{$dst} ) ) {
		if ( $vhostsIPiface{$dst} eq $iface ) {
			...
			my $arpreply = NF2::ARP_pkt->new_reply(
				SA           => $dstMac,
				DA           => $srcMac,
				SenderIpAddr => $dst,
				TargetIpAddr => $src,
...
sub nftest_expect_ICMP_host_unreach {
	...
	return expect_ICMP_dest_unreach( $dst, $src, NF2::ICMP::DEST_HOST_UNREACH, $pkt );
}
...
sub nftest_expect_ICMP_port_unreach {
	...
	return expect_ICMP_dest_unreach( $dst, $src, NF2::ICMP::DEST_PORT_UNREACH, $pkt );
}
...
sub nftest_expect_ICMP_proto_unreach {
	...
	return expect_ICMP_dest_unreach( $dst, $src, NF2::ICMP::DEST_PROTO_UNREACH, $pkt );
}
...
sub nftest_expect_ICMP_network_unreach {
	...
	return expect_ICMP_dest_unreach( $dst, $src, NF2::ICMP::DEST_NET_UNREACH, $pkt );
}
...
sub nftest_expect_ICMP_time_exceeded {
	...
	my $icmppkt = NF2::ICMP_pkt->new_time_exceeded(
		Packet => $pkt,
		src_ip => $src,
	);
...
sub expect_ICMP_dest_unreach {
	...
	# Create the actual ICMP packet
	my $icmppkt = NF2::ICMP_pkt->new_dest_unreach(
		Reason => $reason,
		Packet => $pkt,
		src_ip => $src,
	);
