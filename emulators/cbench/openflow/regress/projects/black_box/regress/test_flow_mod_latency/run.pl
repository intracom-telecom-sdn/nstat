...
sub forward_flow_mod_latency {
	...
	if (1) {
	    ...
	    $flow_mod_pkt = create_flow_mod_from_udp( $ofp, $test_pkt, $in_port, $enums{'OFPP_CONTROLLER'}, 20, $flags, $wildcards );
	...
	for ( my $c = 0 ; $c < 100 ; $c++ ) {
	    ...
	    $flow_mod_pkt = create_flow_mod_from_udp( $ofp, $test_pkt, $in_port, $out_port, 15, $flags, $wildcards );
