...
def sb_idle_mininet_run(out_json,
                        ctrl_base_dir,
                        mininet_base_dir,
                        conf,
                        output_dir,
    ...
    :type out_json: string
    :type ctrl_base_dir: string
    :type mininet_base_dir: string
    :type conf: string
    ...
    mininet_add_switches_handler = mininet_base_dir + \
       conf['mininet_add_switches_handler']
    mininet_remove_switches_handler = mininet_base_dir + \
        conf['mininet_remove_switches_handler']
    ...
    mininet_server_remote_path = \
        '~/transfered_files/mininet/mininet_custom_boot.py'
    ...
    try:
        ...
        util.netutil.create_remote_directory(mininet_ip, mininet_username,
            mininet_password, '/home/vagrant/transfered_files/',
            mininet_ssh_port)

        logging.info(
            '{0} Copying mininet emulator directory to Mininet VM.'.
            format(test_type))

        util.netutil.copy_directory_to_target(mininet_ip, mininet_username,
            mininet_password, mininet_base_dir,
            '/home/vagrant/transfered_files/', mininet_ssh_port)
        ...
        # of all handlers
        common.check_filelist([controller_build_handler,
            ...
            mininet_add_switches_handler, mininet_remove_switches_handler,
            mininet_stop_switches_handler, mininet_get_switches_handler,
            mininet_start_topo_handler])
        ...
        if profiler != None:
            profiler.setup_karaf_env()
        controller_utils.check_for_active_controller(controller_port)


        logging.debug('{0} Booting up Mininet REST server'.format(test_type))
        mininet_utils.start_mininet_server(mininet_ssh_session,
            mininet_server_remote_path, mininet_ip, mininet_server_rest_port)
        ...
        for (mininet_hosts_per_switch,
            ...
            if profiler != None:
                profiler.opts['recording_name'] = '_'.join([
                                  'sb_idle_mininet',
                                  str(mininet_size),
                                  str(mininet_group_size),
                                  str(mininet_group_delay_ms),
                                  str(mininet_hosts_per_switch),
                                  str(mininet_topology_type),
                                  str(controller_statistics_period_ms)])
                profiler.opts['flamegraph_name'] = profiler.opts['recording_name']
            cpid = controller_utils.start_controller(controller_start_handler,
                controller_status_handler, controller_port,
                controller_cpus_str, profiler)
            ...
            controller_utils.stop_controller(controller_stop_handler,
                controller_status_handler, cpid, profiler)
        ...
        # If something goes wrong, then Mininet topology may not exist and
        # stop_mininet_topo() in this case will fail. Finally block will stop
        # and not continue to the end, having as result an unclean Mininet
        # machine. That is why we put the try block here. We want finally
        # block to go to the end and clean Mininet machine.
        try:
        ...
        logging.debug('{0} Delete handlers from Mininet VM'.format(test_type))
...
def get_report_spec(test_type, config_json, results_json):
    ...
    :param test_type (string): Describes the type of the specific test.
    Basically this value defines the Title of the html report.
    :param config_json: This is the filepath to the configuration
    json file.
    :param results_json: This is the filepath to the results json
    file.
    ...
    :type: config_json: string
    :type: results_json: string
    """
    ...
    report_spec_obj = report_spec.ReportSpec(config_json, results_json,
        '{0}'.format(test_type), [report_spec.TableSpec('1d',
            ...
            [('test_repeats', 'Test repeats'),
             ...
             ('mininet_add_switches_handler', 'Mininet add switches handler'),
             ('mininet_remove_switches_handler',
              'Mininet remove switches handler'),
        ...
        [report_spec.TableSpec('2d', 'Test results',
            [('global_sample_id', 'Sample ID'),
             ...
             ('mininet_group_delay_ms', 'Mininet Group Delay'),
