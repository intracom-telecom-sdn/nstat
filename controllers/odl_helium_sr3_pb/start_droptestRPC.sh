...
# Starts Helium SR3 with dropAllPacketsRPC on
...
exec_client_command "dropAllPacketsRPC on"
echo "dropAllPacketsRPC: set [ON] successfully"
...
until [ "$state" == "Active" ]
do
    state=$(./client "bundle:list" | grep "drop-test" | awk '{print $3}')
    sleep 1
done
